...
func (p *Package) SavePackage(db *gorm.DB) (*Package, error) {
	newPackage := Package{}
	err := db.Debug().Model(&Package{}).Where("sender_id = ? AND recipient_id = ? AND tracking = ? AND tracking IS NOT NULL", p.SenderID, p.RecipientID, p.Tracking).Attrs(Package{MailCarrierID: p.MailCarrierID, SenderID: p.SenderID, RecipientID: p.RecipientID, Tracking: p.Tracking, CreatedAt: time.Now()}).FirstOrCreate(&newPackage).Error
	if err != nil {
		return &Package{}, err
	}
...
func (p *Package) UpdatePackage(db *gorm.DB, uid uuid.UUID, tracking string, delivered bool, deliveredOn time.Time) (*Package, error) {

	var err error
	err = db.Debug().Model(&Package{}).Where("tracking = ? AND (sender_id = ? OR recipient_id = ?)", tracking, uid, uid).Updates(Package{Delivered: delivered, DeliveredOn: null.TimeFrom(deliveredOn), UpdatedAt: time.Now()}).Error
	if err != nil {
		return &Package{}, err
	}
	return p, nil
...
func (p *Package) FindAllOpenPackagesForUser(db *gorm.DB, uid uuid.UUID) (*[]Package, error) {
	var err error
	packages := []Package{}
	err = db.Debug().Set("gorm:auto_preload", true).Model(&Package{}).Where("(sender_id = ? OR recipient_id = ?) AND tracking IS NOT NULL AND tracking <> '' AND delivered = false", uid, uid).Limit(100).Find(&packages).Error
	if err != nil {
		return &[]Package{}, err
	}
	return &packages, nil
...
func (p *Package) FindAllDeliveredPackagesForUser(db *gorm.DB, uid uuid.UUID) (*[]Package, error) {
	var err error
	packages := []Package{}
	err = db.Debug().Set("gorm:auto_preload", true).Model(&Package{}).Where("(sender_id = ? OR recipient_id = ?) AND tracking IS NOT NULL AND tracking <> '' AND delivered = true", uid, uid).Limit(100).Find(&packages).Error
	if err != nil {
		return &[]Package{}, err
	}
	return &packages, nil
